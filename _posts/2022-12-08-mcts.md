...
Monte-Carlo Tree Search (MCTS) is an algorithm for exploring a tree of states. When we sample a lot of episodes, many of them will have the same beginning (because there are exponentially more possibilities per timestep, depending on the state branching factor and theirfore the beginning of episodes is probably shared by multiple episodes).

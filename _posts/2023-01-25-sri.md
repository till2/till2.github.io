...
Eine weitere Annahme ist, dass Populationsgröße $N$ gefixt ist. Die Agenten besuchen alle Gebäude außer der Mensa und Bibliothek (die beiden Ziele werden leicht bevorzugt) mit einer gleichverteilten Wahrscheinlichkeit.


Die Wahrscheinlichkeit, dass eine Person eines der 30 Gebäude als nächstes Ziel $X$ auswählt, wird dann wie folgt berechnet:
...
$$
28 \times \frac{1}{35} + 2 \times \frac{1}{10} = 1.
$$

Man könnte noch modellieren, dass tatsächlich Personen mit dem Zug fahren; allerdings haben wir für unser Projekt hier eine Grenze für den Entwicklungsaufwand gezogen und dieses Feature nicht implementiert.
...
Die Änderungsraten im SIR-Gleichungssystem lauten wie folgt [9] [10]:
...
Im klassischen SEIR-Modell wird die Anzahl der Zustandswechsel der Partikel mit Raten ausgerechnet [9]. Wir benutzen für unsere Partikelsimulation stattdessen Wahrscheinlichkeiten, die ausdrücken, wie wahrscheinlich es ist, dass ein Partikel in einem Zeitschritt seinen Zustand wechselt:

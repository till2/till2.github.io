...
&=
\begin{bmatrix} 
	R^{\pi}_{s_1} + \gamma \sum_i P^{\pi}_{s_1 s'_i} V^{\pi}_{s_i} \\
    \vdots \\
    R^{\pi}_{s_n} + \gamma \sum_i P^{\pi}_{s_n s'_i} V^{\pi}_{s_i} \\
\end{bmatrix}
\end{align*}
$$
...
If $\|S\|^2$ is too large to fit in main memory (this will be the case for the most MDPs, often times we deal with huge state spaces), we can use value iteration:
